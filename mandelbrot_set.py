...
res_image = None
palette = [ImageColor.getrgb(
    f"hsl({((i / colorres) / 256 * 360) % 360},100%,{0.000000000045 * (i / colorres - 256) ** 5 + 50}%)") for i in
           range(256 * colorres)]
    ...
    def click(event):
        ...
        if not computing:
            ...
            threading.Thread(None,
                             lambda: set_image(image, canvas, center, list(map(lambda x: x / zoom, (5, 3))),
                                               iter)).start()
